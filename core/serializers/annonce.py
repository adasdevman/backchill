...
class AnnonceSerializer(TimeStampedModelSerializer):
    ...
    def update(self, instance, validated_data):
        ...
        # Gérer les photos s'il y en a de nouvelles
        photos = self.initial_data.get('photos', [])
        if photos:
            # Supprimer les photos existantes seulement si explicitement demandé
            # instance.photos.all().delete()
            
            # Créer de nouvelles photos
            for photo_url in photos:
                # Vérifie si cette photo existe déjà
                if not instance.photos.filter(image=photo_url).exists():
                    try:
                        ...
                        print(f"Erreur lors de l'ajout de la photo {photo_url}: {str(e)}")

        # Gérer l'URL de la vidéo si présente
        video_url = self.initial_data.get('videoUrl')
        if video_url:
            # Supprimer les anciennes vidéos
            instance.videos.all().delete()
            
            try:
                GalerieVideo.objects.create(
                    annonce=instance,
                    video=video_url
                )
                print(f"Vidéo ajoutée avec succès: {video_url}")
            except Exception as e:
                print(f"Erreur lors de l'ajout de la vidéo {video_url}: {str(e)}")
...
class AnnonceDetailSerializer(TimeStampedModelSerializer):
    ...
    tarifs = TarifSerializer(many=True, read_only=True)
    photos = GaleriePhotoSerializer(many=True, read_only=True)
    videos = GalerieVideoSerializer(many=True, read_only=True)  # Added videos field
    annonceur = UserProfileSerializer(source='utilisateur', read_only=True)
    status = serializers.CharField(read_only=True)
    date_evenement = serializers.SerializerMethodField()
    heure_evenement = serializers.SerializerMethodField()
    # Add a serializer method field for video URL for easy access in frontend
    videoUrl = serializers.SerializerMethodField()
    
    def get_videoUrl(self, obj):
        """
        Return the URL of the first video if it exists
        """
        if obj.videos.exists():
            return obj.videos.first().video.url
        return None
    ...
    class Meta:
        ...
        fields = [
            ...
            'videos', 'created', 'modified', 'annonceur', 'status'
        ]
        read_only_fields = ['utilisateur']
